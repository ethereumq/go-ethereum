...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and  Ethereum Quantum address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an  Ethereum Quantum transaction after having the user validate the parameters
...
// offline returns whether the wallet and the  Ethereum Quantum app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		//  Ethereum Quantum app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the  Ethereum Quantum app's version, will fail prior to v1.0.2
...
// and returning the  Ethereum Quantum address located on that derivation path.
...
//
// Note, if the version of the  Ethereum Quantum application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the  Ethereum Quantum app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the  Ethereum Quantum wallet app running
...
// ledgerDerive retrieves the currently active  Ethereum Quantum address from a Ledger
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the  Ethereum Quantum hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an  Ethereum Quantum address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the  Ethereum Quantum signature and do a sanity validation
	if len(reply) != 65 {
