...
import (
	...
	"github.com/ethereumq/go-ethereumq/accounts/keystore"
	...
	"github.com/ethereumq/go-ethereumq/ethstats"
	"github.com/ethereumq/go-ethereumq/les"
...
var (
	...
	ethPortFlag = flag.Int("ethport", 40303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the  Ethereum Quantum protocol")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for  Ethereum Quantum and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` //  Ethereum Quantum address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an  Ethereum Quantum light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          //  Ethereum Quantum protocol stack
	client *ethclient.Client   // Client connection to the  Ethereum Quantum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the  Ethereum Quantum light client protocol
...
// close terminates the  Ethereum Quantum connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the  Ethereum Quantum address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/ethereumq/go-ethereumq/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and  Ethereum Quantum address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for  Ethereum Quantum addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No  Ethereum Quantum address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the  Ethereum Quantum address and profile URL.
...
// returning the username, avatar URL and  Ethereum Quantum address to fund on success.
