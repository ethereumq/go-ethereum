...
The dashboard is a data visualizer integrated into geth, intended to collect and visualize useful information of an  Ethereum Quantum node. It consists of two parts:
...
Normally the dashboard assets are bundled into gethq via `go-bindata` to avoid external dependencies. Rebuilding gethq after each UI modification however is not feasible from a developer perspective. Instead, we can run `webpack` in watch mode to automatically rebundle the UI, and ask `geth` to use external assets to not rely on compiled resources:

```
$ (cd dashboard/assets && ./node_modules/.bin/webpack --watch)
$ gethq --dashboard --dashboard.assets=dashboard/assets/public --vmodule=dashboard=5
```
