...
// nodeDockerfile is the Dockerfile required to run an  Ethereum Quantum node.
var nodeDockerfile = `
...
// an  Ethereum Quantum node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new  Ethereum Quantum node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gethq --exec admin.nodeInfo.id attach", network, kind)); err != nil {
