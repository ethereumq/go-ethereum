...
#
# The cluster contains a bootnode, a GetHQ node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the GetHQ and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a GetHQ node with --datadir pointing at <base-dir>/geth
...
start_geth_node() {
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get GetHQ account address"
  fi
...
create_account() {
  ...
  GetHQ --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
