...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/ethereumq                                                            && \
	(cd $GOPATH/src/github.com/ethereumq && git clone --depth=1 https://github.com/ethereumq/go-ethereumq) && \
  go build -v github.com/ethereumq/go-ethereumq/cmd/faucet                                              && \
